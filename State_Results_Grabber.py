# Author: Adele Bosma
# Purpose: Grabs all state data from the UIL website for the given year and saves it to a csv file.
...
# you want to make sure SEASON_ID is set to 18. Then, run the script and it will create a folder for each conference and save the state results in that folder.
# The naming conventions for the files are Results_{conference}A_State.csv. For example, the results for conference 1A will be saved as 
# Results_1A_State.csv in the Results_1A folder.
import pandas as pd
...
for conference in range(1, CONFERENCE_COUNT + 1):
    ...
    for grouping_id in GROUPING_IDS:
        ...
        xhtml = urlGetContents(f'https://postings.speechwire.com/r-uil-academics.php?groupingid={grouping_id}&Submit=View+postings&region=&district=&state=1&conference={conference}&seasonid={SEASON_ID}').decode('utf-8')
        p = HTMLTableParser()
        p.feed(xhtml)
